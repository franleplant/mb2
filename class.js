...
Game.prototype.beginGame = function(game){
	this.loadLvl();
...
Game.prototype.jugadorSeMovio = function(dx){
	this.d += dx;
...
//VIEWABLE
//--------------------------------------------------------
function Viewable(x,y,img,game){
	this.pos = new XY(x,y);
...
//STATIC
//--------------------------------------------------------
function Static(x,y,w,h,img,game){
	Viewable.call(this, x,y,img,game);
	this.size = new Size(w,h);
}
Static.prototype = new Viewable;
//--------------------------------------------------------
...
function Floor(x,w,img,game){
	Static.call(this, x,game.size.h - celda,w,celda,img,game);
	this.parts = this.createParts(img);
}

Floor.prototype = new Static;

Floor.prototype.createParts = function(img){
	var out = [];
	for(var i = 0; i<this.size.w ;i++){		
		out[i] = new Static(this.pos.x + celda*i, this.pos.y, celda, this.size.h, img, this.game);
	};
	return out;
};
